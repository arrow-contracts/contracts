...
/// @title ArrowBondingCurveConfig
/// @notice Same mechanics as ArrowBondingCurve (native-quote), except VIRTUAL_ETH and
///         MIGRATION_THRESHOLD are constructor-set `immutable`s instead of hardcoded
///         `constant`s — same pattern ArrowBondingCurveStable already uses for its
///         quote token amounts, applied here to native-quote chains too.
/// @dev Why this exists: "4 ether" as a fixed migration bar makes sense on a chain
///      whose native token is worth thousands of dollars (ETH, BNB) but not on one
///      whose native token is worth single-digit dollars (AVAX at ~$7 makes "4 AVAX"
///      a ~$28 raise — migrates almost immediately, no real price discovery). Every
///      other field/behavior is identical to ArrowBondingCurve.
contract ArrowBondingCurveConfig is ReentrancyGuard {
    ...
    constructor(address factory_, address creator_, address router_, uint256 virtualEth_, uint256 migrationThreshold_) {
        factory = factory_;
        creator = creator_;
